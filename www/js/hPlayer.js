...
var hPlayer = function() {
    ...
    var hPlugin = {
		...
		prev:function() {
			hApi.togglePlaylist("prev");
        },
		next:function() {
            hApi.togglePlaylist("next");
        },
        mute:function() {
			var vBut = $('#playerControl-4');
...
/* 					 if(hOptions.init.ad_state){
			...
			addPlaylist:function() {
                var plugin = this.__getPlugin();
				var options = this.get();
				var _playlist = hOptions.init.stream;
				
				if(_playlist instanceof Array) {
			...
			getPluginInfo:function() {
				var plugin = this.__getPlugin();
				var plugin_url = 'http://www.videolan.org/vlc/';
				if (plugin.versionInfo!==undefined) {
					return plugin.versionInfo()
				} else {
					...
					msgBox({
						...
						buttons:{
							btn1:{
								btn_name:'Скачать',btn_class:'dwnload',btn_event:function (btn_name){window.open('/files/vlc-2.1.5-win32.exe','_blank')}
							},
							btn2:{btn_name:'Закрыть',btn_event:'close_msgBox'}
						}
					});
					return false;
				};
			...
			countPlaylist:function() {
				var plugin = this.__getPlugin();
				return plugin.playlist.items.count;
            },
			togglePlaylist:function(direction) {
                var plugin = this.__getPlugin();
    ...
    function interfacePart(wrapper) {
        var hFace = {
            n_main: wrapper,
            n_screen: wrapper + '_screen',
            n_plugin: wrapper + '_plugin',
            n_infoBox: wrapper + '_infoBox',
            n_toolbar: wrapper + '_toolbar',
			n_timeline: wrapper + '_timeline',


            e_main: $('#' + wrapper),
            e_screen: $('#' + wrapper + '_screen'),
            e_plugin: $('#' + wrapper + '_plugin'),
            e_infoBox: $('#' + wrapper + '_infoBox'),
            e_toolbar: $('#' + wrapper + '_toolbar')
...
//			console.log(tsWidth);
        ...
        initInterface:function() {
          ...
          _toolbar.append(_controlContiner);
			...
			hPlugin.createButton('playerControl', 'Назад', 'ico-fast-backward', function(event) {hPlugin.prev();});
			hPlugin.createButton('playerControl', 'Вперед', 'ico-fast-forward', function(event) {hPlugin.next();});
